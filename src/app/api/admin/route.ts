...
const playerNames = ["Notch", "Herobrine", "Steve", "creeper_fan99", "DiamondKing_"];
let chatCounter = chatHistory.length;

export async function GET(request: NextRequest) {
  const authHeader = request.headers.get("x-admin-token");
  // In production, validate a real token
  // We'll accept any non-empty token for demo
  ...
  const stream = new ReadableStream({
    start(controller) {
      function send() {
        ...
        // Occasionally add a new chat message
        if (Math.random() < 0.4) {
          chatCounter++;
          chatHistory.push({
            ...
            timestamp: Date.now(),
            type: Math.random() < 0.85 ? "chat" : (Math.random() < 0.5 ? "join" : "leave"),
          });
        ...
        const data = {
          ...
          metrics: {
            chunksLoaded: Math.floor(450 + Math.sin(Date.now() / 20000) * 50),
            entitiesLoaded: Math.floor(1200 + Math.sin(Date.now() / 18000) * 200),
            dayTime: Math.floor((Date.now() / 1000) % 24000),
            weather: Math.sin(Date.now() / 50000) > 0.7 ? "rain" : "clear",
          },
        };
